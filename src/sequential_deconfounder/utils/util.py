...
def _get_elbo_components(model_obj, A_batch, X_batch, obs_mask=None):
    """
    ...
    guide_trace = poutine.trace(model_obj.guide).get_trace(A_batch, X_batch, obs_mask=obs_mask)
    guide_trace.compute_log_prob()   
    
    model_trace = poutine.trace(poutine.replay(model_obj.model, trace=guide_trace)).get_trace(
        A_batch, X_batch, obs_mask=obs_mask
    )
...
def train_dvae_with_diagnostics(
    ...
    for epoch in range(num_epochs):
        ...
        def model_with_beta(A, X, M=None):
            return model_obj.model(A, X, obs_mask=M, beta=beta)
        ...
        for i in range(0, n, batch_size):
            ...
            with torch.no_grad():  
                elbo_val, recon_nll, kl_val, post_var = _get_elbo_components(
                    model_obj, A_b, X_b, obs_mask=None if obs_mask is None else obs_mask[idx]
                )
            epoch_elbo += elbo_val
        ...
        logs.append({
            ...
            "recon_nll": avg_recon,
            "kl": avg_kl,
            "kl_weighted": beta * avg_kl,     # ← Penalized KL
